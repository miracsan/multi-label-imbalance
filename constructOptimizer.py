...
def construct_optimizer(model, method, criterion, optim_config='optimizer_config_1', learning_rate=0.01, weight_decay=5e-4):
    param_iter = filter(lambda p: p.requires_grad, model.parameters())
    
    if 'multitask' in method:
...
def load_optimizer(model, checkpoint_path):
    ...
    optim_config = checkpoint['optimizer'] if 'optimizer' in checkpoint else 'optimizer_config_1'
...
def adjust_optimizer_for_new_hyperparams(optimizer, learning_rate=0.01, weight_decay=5e-4):
