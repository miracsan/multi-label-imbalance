...
class Eye:
    ...
    def __call__(self, target):
        return np.eye(self.num_classes, dtype=np.uint8)[np.array(target)].astype(np.bool)
...
class MinMaxNormalize:
    ...
    def __call__(self, image):
        if self.num_channels > 1:
            transposed_image = np.swapaxes(image, 0, -1); #Assumes the last dimension of input to be channel
            
            image_dimensions = len(np.shape(transposed_image)) - 1
            shape_to_cast = tuple(image_dimensions * [1]);
            
            min_vals_per_channel = np.array(list(map(np.min, transposed_image))).reshape(-1,*shape_to_cast); #Mins of each channel (axis 0)
            max_vals_per_channel = np.array(list(map(np.max, transposed_image))).reshape(-1,*shape_to_cast);
            
            normalized_transposed_image = (transposed_image-min_vals_per_channel) / (max_vals_per_channel-min_vals_per_channel+self.smooth)
...
class Standardize:
    ...
    def __call__(self, image):
        if self.num_channels > 1:
            ...
            standardized_transposed_image = (transposed_image-mean_vals_per_channel) / (std_vals_per_channel + self.smooth)
...
class CenterCrop:
    ...
    def __call__(self, image):
        if len(self.crop_size)==3:
            ...
            height_crop, width_crop, depth_crop = self.crop_size[0], self.crop_size[1], self.crop_size[2]
            
            if height_crop != 'all':
                height_min, height_max = max(height//2 - height_crop//2, 0), min(height//2 + height_crop//2, height)
            else:
                height_min, height_max = (0, height)
                
            if width_crop != 'all':
                width_min, width_max = max(width//2 - width_crop//2, 0), min(width//2 + width_crop//2, width)
            else:
                width_min, width_max = (0, width)
                
            if depth_crop != 'all':
                depth_min, depth_max = max(depth//2 - depth_crop//2, 0), min(depth//2 + depth_crop//2, depth)
            else:
                depth_min, depth_max = (0, depth)
            
            cropped_image = image[height_min:height_max,
                                  width_min:width_max,
                                  depth_min:depth_max,
                                  ...]
