...
def make_pred_multilabel(model, dataloaders, method, alias, save_as_csv=False):
    """
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            if ("anchor" in method) or ("ldam" in method):
    ...
    for column in true_df:
        ...
        if column not in dataset.PRED_LABEL:
                    continue
    ...
    if save_as_csv:
        pred_df.to_csv(os.path.join("results", alias, "preds.csv"), index=False)
        auc_df.to_csv(os.path.join("results", alias, "aucs.csv"), index=False)
        true_df.to_csv(os.path.join("results", alias, "true.csv"), index=False)
