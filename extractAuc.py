...
def extract_auc_scores(alias, thr_type='test', cov_checkpoints=90):
  ...
  with open(auc_logpath, 'a') as logfile:
    ...
    if auc_log_exists:
      ...
      logwriter.writerow(["method", "thr_type", "end_coverage",
                          *[f"mAUC_{class_num}" for class_num in range(num_classes)], "mean_foreground_mAUC",
                          "calibration_MSE"])
    ...
    for method in method_list:
      method_df = df[df['method'] == method].drop('method', axis=1).reset_index(drop=True)
      for cov in cov_checkpoints:
        ...
        dice_midpoints = (dices[1:,] + dices[:-1,]) / 2;

        areas_under_curve = coverage_diffs.reshape(-1,1) * dice_midpoints

        sum_aucs = np.sum(areas_under_curve, axis=0)
        normalised_aucs = sum_aucs / (coverages[0] - cov) * 100 #100 is the perfect score
        normalised_aucs = np.hstack((normalised_aucs, np.mean(normalised_aucs[1:])))
        
        

        logwriter.writerow([method, thr_type, int(cov), *np.round(normalised_aucs, 4), np.linalg.norm(discalibration, 2)])
